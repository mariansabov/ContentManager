...
namespace ContentManager.Application.Features.Publications.News
{
    ...
    public class GetAllNewsQueryHandler(IApplicationDatabaseContext context, IMapper mapper) : IRequestHandler<GetAllNewsQuery, List<NewsPublicationDto>>
    {
        public async Task<List<NewsPublicationDto>> Handle(GetAllNewsQuery request, CancellationToken cancellationToken)
        {
            var newsEntities = await context.Publications
                .Where(p => p.Type == PublicationType.News && (p.ExpiresAt == null || p.ExpiresAt > DateTime.UtcNow))
